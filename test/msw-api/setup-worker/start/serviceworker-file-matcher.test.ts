...
test('resolves the "start" Promise and returns a ServiceWorkerRegistration when using a serviceWorkerFileMatcher that returns true', async () => {
  const runtime = await runBrowserWith(
    path.resolve(__dirname, 'serviceworker-file-matcher.mocks.ts'),
  )
...
test('fails to return a ServiceWorkerRegistration when using a serviceWorkerFileMatcher that returns false', async () => {
  const runtime = await runBrowserWith(
    path.resolve(__dirname, 'serviceworker-file-matcher.error.mocks.ts'),
  )
